<x-layout.default>
    ...
    <div class="panel mt-6">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
            <h5 class="text-2xl font-bold text-gray-800">{{ $pageTitle ?? __('Thana') }}</h5>
        ...
        <div>
            <form method="POST" action="{{ $function_type == 'update' ? route('thana.update', isset($item) ? $item->id : '') : route('thana.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div class="mb-2">
                        <label for="district_code" class="">{{ __('District') }}</label>
                        <div class="flex">
                            {!! defaultInputIcon() !!}
                            <select id="district_code" name="district_code" class="form-input ltr:rounded-l-none rtl:rounded-r-none" required>
                                <option value="">{{ __('Select District') }}</option>
                                @if(isset($districts))
                                    @foreach($districts as $district)
                                        <option value="{{ $district->code }}"
                                            @if(isset($item) && $item->district_code == $district->code) selected
                                            @elseif(old('district_code') == $district->code) selected @endif>
                                            {{ $district->name }}
                                        </option>
                                    @endforeach
                                @endif
