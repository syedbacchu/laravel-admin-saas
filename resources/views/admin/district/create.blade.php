<x-layout.default>
    ...
    <div class="panel mt-6">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
            <h5 class="text-2xl font-bold text-gray-800">{{ $pageTitle ?? __('District') }}</h5>
        ...
        <div>
            <form method="POST" action="{{ $function_type == 'update' ? route('district.update', isset($item) ? $item->id : '') : route('district.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div class="mb-2">
                        ...
                        <div class="flex">
                            {!! defaultInputIcon() !!}
                            <input id="name" name="name" type="text" maxlength="180"
                    ...
                    <div class="mb-2">
                        <label for="division_code" class="">{{ __('Division') }}</label>
                        <div class="flex">
                            {!! defaultInputIcon() !!}
                            <select id="division_code" name="division_code" class="form-input ltr:rounded-l-none rtl:rounded-r-none" required>
                                <option value="">{{ __('Select Division') }}</option>
                                @if(isset($divisions))
                                    @foreach($divisions as $division)
                                        <option value="{{ $division->code }}"
                                            @if(isset($item) && $item->division_code == $division->code) selected
                                            @elseif(old('division_code') == $division->code) selected @endif>
                                            {{ $division->name }}
                                        </option>
                                    @endforeach
                                @endif
                    ...
                    <div class="mb-4">
                        <label for="status" class="block text-gray-700 font-medium mb-2">{{ __('Activation Status') }}</label>
                        <div class="flex">
                            {!! defaultInputIcon() !!}
