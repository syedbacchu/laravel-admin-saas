    ...
    <div class="panel mt-6">
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-indigo-600 to-blue-600 px-6 py-4">
                <h2 class="text-xl font-bold text-white">{{ $componentModel->name }}</h2>
                <p class="text-indigo-200 text-sm mt-1">{{ __('Slug:') }} {{ $componentModel->slug }}</p>
            </div>

            <!-- Component Details -->
            <div class="p-6">
                <!-- Description -->
                @if($componentModel->description)
                <div class="mb-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-2">{{ __('Description') }}</h3>
                    <p class="text-gray-600">{{ $componentModel->description }}</p>
                </div>
                @endif
                ...
                <div class="mb-6">
                    ...
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                        {{ $componentModel->status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        {{ $componentModel->status ? __('Active') : __('Inactive') }}
                    </span>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <p class="text-gray-600">{{ $componentModel->fields->count() }} {{ __('field(s) configured') }}</p>
                </div>

                <!-- Fields Preview -->
                @if($componentModel->fields->count() > 0)
                <div class="mb-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-2">{{ __('Field Structure') }}</h3>
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                        <ul class="space-y-2">
                            @foreach($componentModel->parentFields as $field)
                ...
                <div class="border-t border-gray-200 pt-6">
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <span class="text-gray-700 ml-2">{{ $componentModel->created_at ? $componentModel->created_at->format('M d, Y') : '-' }}</span>
                        </div>
                        <div>
                            <span class="text-gray-500">{{ __('Updated:') }}</span>
                            <span class="text-gray-700 ml-2">{{ $componentModel->updated_at ? $componentModel->updated_at->format('M d, Y') : '-' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end mt-6 pt-6 border-t border-gray-200">
                    ...
                    <a href="{{ route('component.edit', $componentModel->id) }}"
